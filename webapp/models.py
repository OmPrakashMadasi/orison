...
class Order(models.Model):
    ...
    total_price = models.DecimalField(max_digits=20, decimal_places=2, default=0.00)  # Total order cost
...
class OrderItem(models.Model):
    ...
    price = models.DecimalField(max_digits=20, decimal_places=2)  # Price at time of order

    def total_price(self):
        return self.price * self.quantity

    def __str__(self):
        return f"{self.quantity}x {self.product.name} ({self.size.size if self.size else 'No Size'})"
